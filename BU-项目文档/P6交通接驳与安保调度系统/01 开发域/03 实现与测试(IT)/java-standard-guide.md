...
1.我们参考 Kent Beck 的简单设计四原则来指导我们的如何写出优秀的代码，如何有效地判断我们的代码是优秀的。
...
同时代码的行文格式，应尽可能保持前后一致，过长的方法、模块应当拆分为更易于理解阅读的结构。
模块、方法间应当尽可能降低耦合，避免后期维护需要查看过多的方法，而完整的一个内部流程应当保证具有高聚合性。

2.计算/数据缺陷
    2-1.计算时，应避免不同类型数据之间的混合计算导致出现精度丢失等情况。
    2-2.编写数据计算方法时，应优化结构顺序，并使用括号来避免后期回看时可能的理解错误。
    2-3.数据传入时，需要确认IO异常处理等是否合理，根据需求判断是否抛出异常。
    2-4.对象、数组被传递转换时，应保证数据不会因为转换出现精度丢失、数据缺失等问题。
    2-5.尽量保存计算后的数值，避免重复计算浪费系统性能。

3.进行布尔值对比时，应确认判断值是否为空，假设判断条件为多个，需要注意判断触发的先后顺序。
    
4.流程控制缺陷
    4-1.所有涉及循环的方法应当尽量优化，选择最佳的循环结构。
    4-2.必须避免死循环方法，若循环有多个出口，则必须设置对应的传值和默认值，对可能的异常进行正确的处理。
    4-3.必须避免超过三层的过深循环嵌套，若有必要，可以拆成多个循环。
...
3.方法名描述方法应与命名约定一致，避免混乱。
4.通用的方法类应尽可能作为public或protected放在父类中，提高类的复用性。
5.涉及IO传输时，所有打开、新建的流都应该在结束使用时关闭。

### 5、参数规范

1.变量和常量命名需与约定保持一致，保证书写正确；变量名与属性名尽量避免过于相似，容易导致混淆。
2.所有变量再被调用时，需确认是否已经被正确地声明和初始化，并且考虑判空和数组超范围的情况。
3.常用的文字常量尽量设为常量，减少魔法值的出现，便于常量管理。
4.尽量在方法内使用局部变量来取代全局变量，避免相互影响，若必须使用全局变量，在方法调用前必须进行检查。
5.所有涉及实际单位的参数，尽可能回传值内添加对应的单位信息，避免度量单位不一致的情况出现。
...
> _对于@Transactional 注解，当 spring 遇到该注解时，会自动从数据库连接池中获取 connection，并开启事务然后绑定到 ThreadLocal 上，如果业务并没有进入到最终的 操作数据库环节，那么就没有必要获取连接并开启事务，应该直接将 connection 返回给数据库连接池，供其他使用（比较难以讲解清楚，如果不懂的话就主动去问）。
